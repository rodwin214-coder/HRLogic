/*
  # Restore Payroll Functions as SECURITY DEFINER with Internal Guards

  ## Problem
  The payroll tables (payroll_adjustments, payroll_records, payroll_periods) use
  RLS policies that check current_setting('app.current_company_id'), which is a
  session variable. SECURITY INVOKER functions run as the caller's Postgres role
  and cannot bypass RLS, so the session variable check fails and all reads/writes
  are blocked.

  ## Solution
  Restore functions to SECURITY DEFINER so they run as the DB owner and bypass
  RLS. Data isolation is enforced by:
    1. Internal p_company_id scoping on every query (WHERE company_id = p_company_id)
    2. EXECUTE permission restricted to authenticated role only (anon is revoked)
    3. Functions like add_payroll_adjustment verify period and employee belong to
       the given company before inserting

  This is the correct and standard Supabase pattern for functions that need to
  operate on RLS-protected tables.

  ## Functions Restored to SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.get_payroll_adjustments(uuid, uuid) FROM PUBLIC, anon;
...
REVOKE ALL ON FUNCTION public.add_payroll_adjustment(uuid, uuid, uuid, text, numeric, text) FROM PUBLIC, anon;
...
REVOKE ALL ON FUNCTION public.create_payroll_period(uuid, text, text, date, date, date, text) FROM PUBLIC, anon;
...
REVOKE ALL ON FUNCTION public.update_payroll_period_status(uuid, uuid, text) FROM PUBLIC, anon;
...
REVOKE ALL ON FUNCTION public.delete_payroll_period(uuid, uuid) FROM PUBLIC, anon;
